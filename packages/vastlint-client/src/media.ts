import type {
  VastMediaFile,
  VastMediaSelectionCandidate,
  VastMediaSelectionOptions,
  VastMediaSelectionResult,
  VastResolvedAd,
} from "./types.js";
...
function buildPreferredIndex(values: readonly string[] | undefined): Map<string, number> {
  return new Map((values ?? []).map((value, index) => [normalizeValue(value), index]));
}

function sortCandidates(left: VastMediaSelectionCandidate, right: VastMediaSelectionCandidate): number {
  return right.score - left.score || left.mediaFile.url.localeCompare(right.mediaFile.url);
}

function scoreDistance(target: number | undefined, actual: number | null, weight: number): number {
...
function scoreMediaFile(
  mediaFile: VastMediaFile,
  options: VastMediaSelectionOptions,
  preferredMimeTypes: Map<string, number>,
  preferredDelivery: Map<string, number>,
): VastMediaSelectionCandidate | null {
...
export function rankMediaFiles(
  mediaFiles: readonly VastMediaFile[],
  options: VastMediaSelectionOptions = {},
): VastMediaSelectionCandidate[] {
  ...
  return mediaFiles
    ...
    .filter((candidate): candidate is VastMediaSelectionCandidate => candidate !== null)
    .sort(sortCandidates);
}
...
export function selectResolvedAdMediaFile(
