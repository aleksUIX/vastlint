...
function extractSkipOffset(xml: string): string | null {
...
function groupTrackingEvents(plan: VastTrackingPlan): Record<string, string[]> {
  ...
  return Object.fromEntries(
    Object.entries(grouped).map(([event, urls]) => [event, uniqueUrls(urls)]),
  );
}

function cloneMediaFiles(mediaFiles: VastMediaFile[]): VastMediaFile[] {
...
function collectTrackingEventMap(xml: string): Record<string, string[]> {
  const grouped: Record<string, string[]> = {};
...
function collectAdSegments(xml: string): AdXmlSegment[] {
  const segments: AdXmlSegment[] = [];
...
function extractMediaFiles(xml: string): VastMediaFile[] {
...
function extractSequence(rawAttributes: string): number | null {
...
function sortAdSegments(segments: AdXmlSegment[]): AdXmlSegment[] {
...
function buildAdTrackingPlan(
  wrapperChain: readonly VastWrapperHop[],
  finalSegmentXml: string,
): VastTrackingPlan {
  const hops = [
...
function collectOpenCloseElements(
  xml: string,
  tagName: string,
): Array<{ rawAttributes: string; body: string }> {
  const elements: Array<{ rawAttributes: string; body: string }> = [];
  const pattern = new RegExp(`<${tagName}\\b([^>]*)>([\\s\\S]*?)<\\/${tagName}>`, "gi");
...
function collectSelfClosingOrOpenCloseElements(
  xml: string,
  tagName: string,
): Array<{ rawAttributes: string; body: string }> {
  const elements: Array<{ rawAttributes: string; body: string }> = [];
...
function collectCreativeResources(xml: string): VastCreativeResource[] {
  const resources: VastCreativeResource[] = [];
  const resourceTags: Array<{ tagName: string; kind: VastCreativeResource["kind"] }> = [
...
function extractUniversalAdIds(xml: string): VastUniversalAdId[] {
  const universalAdIds: VastUniversalAdId[] = [];
...
function extractCategories(xml: string): VastCategory[] {
...
function extractVerificationResources(xml: string): VastVerificationResource[] {
  const resources: VastVerificationResource[] = [];
  const resourceTags: Array<{ tagName: string; kind: VastVerificationResource["kind"] }> = [
...
function extractAdVerifications(xml: string): VastAdVerification[] {
  return collectOpenCloseElements(xml, "Verification")
    ...
    .filter(
      (verification) => verification.vendor !== null
        || verification.resources.length > 0
        || verification.verificationParameters !== null,
    );
}

function extractAdPodMetadata(xml: string): VastAdPodMetadata {
...
function extractCompanions(xml: string): VastCompanionAd[] {
...
function extractIcons(xml: string): VastIcon[] {
...
function buildResolvedAdFromSegment(
  segment: AdXmlSegment,
  wrapperChain: readonly VastWrapperHop[],
  lastHop: VastWrapperHop,
...
function emptyResolvedAd(stoppedReason: string | null): VastResolvedAd {
  return {
    resolved: false,
    finalHopIndex: null,
    finalUrl: null,
...
export function buildResolvedState(
  ...
  const resolvedAds = adSegments.map((segment) =>
    buildResolvedAdFromSegment(segment, wrapperChain, lastHop, stoppedReason)
  );
...
export function buildResolvedAd(
  wrapperChain: readonly VastWrapperHop[],
  resolution: VastResolutionSummary | null,
): VastResolvedAd | null {
