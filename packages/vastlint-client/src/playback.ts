...
function cloneMediaSelection(snapshot: VastPlaybackSnapshot["mediaSelection"]): VastPlaybackSnapshot["mediaSelection"] {
  return {
...
function buildAdKey(resolvedAd: VastResolvedAd | null): string {
  if (!resolvedAd) {
    return "none";
  }
...
function buildBaseSnapshot(
  sessionSnapshot: VastSessionSnapshot,
  options: VastPlaybackControllerOptions,
): VastPlaybackSnapshot {
...
function clonePlaybackSnapshot(snapshot: VastPlaybackSnapshot, session: VastSession): VastPlaybackSnapshot {
...
function normalizeStatus(
  currentStatus: VastPlaybackStatus,
  sessionSnapshot: VastSessionSnapshot,
...
export function createVastPlaybackController(
  options: VastPlaybackControllerOptions,
): VastPlaybackController {
  const listeners = new Set<(snapshot: VastPlaybackSnapshot) => void>();
  ...
  const setSnapshot = (next: VastPlaybackSnapshot) => {
    snapshot = next;
    notify();
  };
  ...
  const dispatchLifecycleEvent = async (
    ...
    optionsOverride: VastTrackOptions,
  ) => {
  ...
  return {
    ...
    async click(trackOptions = {}) {
      return runAction(async () => {
        ...
        } satisfies VastPlaybackClickResult;
      });
    },
