...
import type {
  VastPlaybackMilestones,
  VastPlaybackQueueClickResult,
  VastPlaybackQueueController,
  VastPlaybackQueueControllerOptions,
  VastPlaybackQueueItem,
...
const PROGRESS_MILESTONES: Array<[keyof VastPlaybackMilestones, number]> = [
  ["firstQuartile", 0.25],
  ["midpoint", 0.5],
  ["thirdQuartile", 0.75],
  ["complete", 1],
...
function createMilestones(): VastPlaybackMilestones {
  return {
    start: false,
    firstQuartile: false,
    midpoint: false,
...
function clonePlaybackMilestones(milestones: VastPlaybackMilestones): VastPlaybackMilestones {
  return { ...milestones };
}

function cloneMediaSelection(item: VastPlaybackQueueItem["mediaSelection"]): VastPlaybackQueueItem["mediaSelection"] {
  return {
...
function buildAdKey(resolvedAd: VastResolvedAd): string {
  return [
    resolvedAd.finalUrl ?? "",
    resolvedAd.adTitle,
    resolvedAd.duration,
...
function buildQueueItem(
  ...
  adIndex: number,
  options: VastPlaybackQueueControllerOptions,
): VastPlaybackQueueItem {
  return {
    adIndex,
    resolvedAd,
...
function syncQueueItem(
  existing: VastPlaybackQueueItem | undefined,
  resolvedAd: VastResolvedAd,
  adIndex: number,
  options: VastPlaybackQueueControllerOptions,
): VastPlaybackQueueItem {
...
function cloneQueueItem(item: VastPlaybackQueueItem): VastPlaybackQueueItem {
  return {
    ...item,
...
function buildQueueItems(
  ...
  options: VastPlaybackQueueControllerOptions,
  previousItems: readonly VastPlaybackQueueItem[] = [],
): VastPlaybackQueueItem[] {
  const previousByKey = new Map(previousItems.map((item) => [buildAdKey(item.resolvedAd), item]));

  return resolvedAds.map((resolvedAd, adIndex) =>
    syncQueueItem(previousByKey.get(buildAdKey(resolvedAd)), resolvedAd, adIndex, options)
  );
}

function normalizeQueueSnapshot(next: VastPlaybackQueueSnapshot): VastPlaybackQueueSnapshot {
  ...
  const status: VastPlaybackStatus = next.session.error
    ? "error"
...
function cloneQueueSnapshot(snapshot: VastPlaybackQueueSnapshot, session: VastSession): VastPlaybackQueueSnapshot {
...
function selectTrackingUrls(resolvedAd: VastResolvedAd, event: Parameters<VastSession["track"]>[0]): string[] {
  if (event === "impression") {
...
export function createVastPlaybackQueueController(
  options: VastPlaybackQueueControllerOptions,
): VastPlaybackQueueController {
  const listeners = new Set<(snapshot: VastPlaybackQueueSnapshot) => void>();
  ...
  const setSnapshot = (next: VastPlaybackQueueSnapshot) => {
  ...
  const updateCurrentItem = (mutator: (item: VastPlaybackQueueItem) => VastPlaybackQueueItem) => {
  ...
  const ensureQueueReady = async (): Promise<VastPlaybackQueueItem> => {
  ...
  const dispatchCurrentEvent = async (
    event: Parameters<VastSession["track"]>[0],
  ...
  return {
    ...
    async click(trackOptions = {}) {
      return runAction(async () => {
        ...
        } satisfies VastPlaybackQueueClickResult;
      });
    },
