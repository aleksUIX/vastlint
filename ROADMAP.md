...
| Milestone | Detail |
|---|---|
| **Core library** | `vastlint-core` v0.7 on crates.io - pure-Rust library, 195 rules, VAST 2.0–4.3 plus SIMID, OMID, VMAP 1.0, and DAAST 1.0 validation. |
...
| **REST API** | Authenticated `/validate` endpoint on [RapidAPI](https://rapidapi.com/aleksUIX/api/vastlint). Same 195 rules, WASM-powered, sub-millisecond response. |
...
| **`vastlint init`** | Generates a starter `vastlint.toml` with all 195 rules listed at their default severities, commented out - a zero-behaviour-change starting point for tuning, and durable evidence of adoption in a repo. |
