...
| **REST API** | Authenticated `/validate` endpoint on [RapidAPI](https://rapidapi.com/aleksUIX/api/vastlint). Same 220 rules, WASM-powered, sub-millisecond response. |
...
| **`vastlint init`** | Generates a starter `vastlint.toml` with all 220 rules listed at their default severities, commented out - a zero-behaviour-change starting point for tuning, and durable evidence of adoption in a repo. |
