...
| Milestone | Detail |
|---|---|
| **Core library** | `vastlint-core` v0.11 on crates.io - pure-Rust library, 223 rules, VAST 2.0–4.4 plus SIMID, OMID, VMAP 1.0, and DAAST 1.0 validation. |
...
| **REST API** | Authenticated `/validate` endpoint on [RapidAPI](https://rapidapi.com/aleksUIX/api/vastlint). Same 223 rules, WASM-powered, sub-millisecond response. |
...
| **`vastlint init`** | Generates a starter `vastlint.toml` with all 223 rules listed at their default severities, commented out - a zero-behaviour-change starting point for tuning, and durable evidence of adoption in a repo. |
